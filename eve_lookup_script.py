...
url_system = r'&usesystem='     # for eve centrals api to find the system to find prices for items i.e. Jita or Rens 
url_typeid =    r'&typeid='
...
def open_for_id_name(id_url):
    ...
    '''
    price_json_object = requests.get(id_url) # lookup the url
...
def lookup_sell(SolarSystem='30000142',ItemID='628'):
    ...
    sell_price = requests.get(sell_string).json() # get the json object from eve central of price data
...
def lookup_buy(SolarSystem='30000142',ItemID='628'):
    ...
    buy_price = requests.get(buy_string).json() # get the json object from eve central of price data
